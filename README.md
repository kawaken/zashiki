...
- **Markdown preview pane** — toggle a preview pane (`Cmd+Shift+M`) that
  ...
  CLI via a custom `zashiki://` URL scheme (e.g. `open
  "zashiki://markdown-preview/open?path=<percent-encoded absolute
  path>"`); the pane live-updates as the file changes. Handy for
