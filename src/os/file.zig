...
///
/// This returns `$TMPDIR`/`$TMP` (or `"/tmp"` as a fallback) without
/// allocating. Always pair with `freeTmpDir` for API symmetry.
...
/// Free a path returned by `allocTmpDir`. This is a no-op on POSIX.
