...
このファイルは、GitHub Issueに`claude`ラベルを付けることをトリガーに
起動する`claude-code-action`実行時にのみ適用される。ローカルでの開発
作業には適用しない。作業を始める前に必ず読み、内容に従うこと。
...
- Issueのclaim(`wip`ラベル・コメント)は通常のフロー通り行う。`wip`ラベルの
  管理はClaude自身の仕事であり、作業中は`wip`ラベルを付け続ける。PRが
  マージされるまで外さない
