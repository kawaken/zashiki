...
extension NSWindow {
    ...
    func addTabbedWindowSafely(
        ...
        var error: NSError?
        let success = ZashikiAddTabbedWindowSafely(self, child, ordered.rawValue, &error)
        if let error {
            Zashiki.logger.error("addTabbedWindow failed: \(error.localizedDescription, privacy: .public)")
        }
