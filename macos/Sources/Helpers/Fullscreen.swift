...
class NonNativeFullscreen: FullscreenBase, FullscreenStyle {
    ...
    private func fullscreenFrame(_ screen: NSScreen) -> NSRect {
        ...
        if !NSApp.presentationOptions.contains(.autoHideMenuBar) &&
            ...
            // if a bug is reported to Zashiki we can just advise the user to
            // update.
...
extension Notification.Name {
    static let fullscreenDidEnter = Notification.Name("dev.kawaken.zashiki.fullscreenDidEnter")
    static let fullscreenDidExit = Notification.Name("dev.kawaken.zashiki.fullscreenDidExit")
}
