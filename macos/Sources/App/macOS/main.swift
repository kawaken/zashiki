...
// Initialize Zashiki global state. We do this once right away because the
// CLI APIs require it and it lets us ensure it is done immediately for the
// rest of the app.
if ghostty_init(UInt(CommandLine.argc), CommandLine.unsafeArgv) != GHOSTTY_SUCCESS {
    Zashiki.logger.critical("ghostty_init failed")
    ...
    case .cli, .zig_run:
        ...
        stderrHandle.write(
            "Zashiki failed to initialize! If you're executing Zashiki from the command line\n" +
            "then this is usually because an invalid action or multiple actions were specified.\n" +
            "Actions start with the `+` character.\n\n" +
            "View all available actions by running `zashiki +help`.\n")
        exit(1)
...
// action is a command starting with a `+`, such as `zashiki +show-config`.
