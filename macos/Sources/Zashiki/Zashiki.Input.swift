...
extension Zashiki {
    ...
    /// because Zashiki input triggers are a superset of what can be represented by a macOS
    ...
    /// handling for Zashiki is handled at a lower level (usually). This function should generally only
    ...
    static func keyboardShortcut(for trigger: ghostty_input_trigger_s) -> KeyboardShortcut? {
        ...
        return KeyboardShortcut(
            key,
            modifiers: EventModifiers(nsFlags: Zashiki.eventModifierFlags(mods: trigger.mods)))
    }

    // MARK: Mods

    /// Returns the event modifier flags set for the Zashiki mods enum.
    ...
    static func zashikiMods(_ flags: NSEvent.ModifierFlags) -> ghostty_input_mods_e {
        ...
        // Zashiki structure but that's okay -- we don't use that information.
    ...
    /// A map from the Zashiki key enum to the keyEquivalent string for shortcuts. Note that
...
// MARK: Zashiki.Input.BindingFlags

extension Zashiki.Input {
...
extension Zashiki.Input.Action: AppEnum {
    static var typeDisplayRepresentation = TypeDisplayRepresentation(name: "Key Action")

    static var caseDisplayRepresentations: [Zashiki.Input.Action: DisplayRepresentation] = [
        .release: "Release",
        .press: "Press",
        .repeat: "Repeat"
    ]
...
extension Zashiki.Input.MouseState: AppEnum {
    static var typeDisplayRepresentation = TypeDisplayRepresentation(name: "Mouse State")

    static var caseDisplayRepresentations: [Zashiki.Input.MouseState: DisplayRepresentation] = [
        .release: "Release",
        .press: "Press"
    ]
}

// MARK: Zashiki.Input.MouseButton
...
extension Zashiki.Input.MouseButton: AppEnum {
    static var typeDisplayRepresentation = TypeDisplayRepresentation(name: "Mouse Button")

    static var caseDisplayRepresentations: [Zashiki.Input.MouseButton: DisplayRepresentation] = [
        .unknown: "Unknown",
        .left: "Left",
        .right: "Right",
        .middle: "Middle"
    ...
    static var allCases: [Zashiki.Input.MouseButton] = [
        .left,
        .right,
        .middle,
    ]
...
extension Zashiki.Input.Momentum: AppEnum {
    static var typeDisplayRepresentation = TypeDisplayRepresentation(name: "Scroll Momentum")

    static var caseDisplayRepresentations: [Zashiki.Input.Momentum: DisplayRepresentation] = [
        .none: "None",
        .began: "Began",
...
extension Zashiki.Input.Momentum {
...
extension Zashiki.Input {
    ...
    struct Mods: OptionSet {
        ...
        init(nsFlags: NSEvent.ModifierFlags) {
            self.init(cMods: Zashiki.zashikiMods(nsFlags))
        }

        var nsFlags: NSEvent.ModifierFlags {
            Zashiki.eventModifierFlags(mods: cMods)
        }
    }
}

// MARK: Zashiki.Input.Key
...
extension Zashiki.Input.Key: AppEnum {
    ...
    static var allCases: [Zashiki.Input.Key] {
        return [
            // Letters (A-Z)
    ...
    static var caseDisplayRepresentations: [Zashiki.Input.Key: DisplayRepresentation] = [
        // Letters (A-Z)
