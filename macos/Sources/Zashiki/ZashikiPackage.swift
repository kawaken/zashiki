...
extension Zashiki {
    ...
    static let userNotificationCategory = "dev.kawaken.zashiki.userNotification"

    // The user notification "Show" action
    static let userNotificationActionShow = "dev.kawaken.zashiki.userNotification.Show"
}
...
extension Zashiki {
    ...
    static var launchSource: LaunchSource {
        guard let envValue = ProcessInfo.processInfo.environment["ZASHIKI_MAC_LAUNCH_SOURCE"] else {
...
extension Zashiki {
    ...
    enum SplitFocusDirection {
        ...
        /// Initialize from a Zashiki API enum.
...
extension Zashiki.SplitFocusDirection {
...
extension Notification.Name {
    ...
    static let zashikiConfigDidChange = Notification.Name("dev.kawaken.zashiki.configDidChange")
    static let ZashikiConfigChangeKey = zashikiConfigDidChange.rawValue

    /// Color change. Object is the surface changing.
    static let zashikiColorDidChange = Notification.Name("dev.kawaken.zashiki.zashikiColorDidChange")
    static let ZashikiColorChangeKey = zashikiColorDidChange.rawValue

    /// Goto tab. Has tab index in the userinfo.
    static let zashikiMoveTab = Notification.Name("dev.kawaken.zashiki.moveTab")
    static let ZashikiMoveTabKey = zashikiMoveTab.rawValue

    /// Close tab
    static let zashikiCloseTab = Notification.Name("dev.kawaken.zashiki.closeTab")

    /// Close other tabs
    static let zashikiCloseOtherTabs = Notification.Name("dev.kawaken.zashiki.closeOtherTabs")

    /// Close tabs to the right of the focused tab
    static let zashikiCloseTabsOnTheRight = Notification.Name("dev.kawaken.zashiki.closeTabsOnTheRight")

    /// Close window
    static let zashikiCloseWindow = Notification.Name("dev.kawaken.zashiki.closeWindow")

    /// Resize the window to a default size.
    static let zashikiResetWindowSize = Notification.Name("dev.kawaken.zashiki.resetWindowSize")

    /// Ring the bell
    static let zashikiBellDidRing = Notification.Name("dev.kawaken.zashiki.zashikiBellDidRing")

    /// The active selection changed
    static let zashikiSelectionDidChange = Notification.Name("dev.kawaken.zashiki.zashikiSelectionDidChange")

    /// Readonly mode changed
    static let zashikiDidChangeReadonly = Notification.Name("dev.kawaken.zashiki.didChangeReadonly")
    static let ReadonlyKey = zashikiDidChangeReadonly.rawValue + ".readonly"
    static let zashikiCommandPaletteDidToggle = Notification.Name("dev.kawaken.zashiki.commandPaletteDidToggle")

    /// Toggle maximize of current window
    static let zashikiMaximizeDidToggle = Notification.Name("dev.kawaken.zashiki.maximizeDidToggle")
    ...
    static let zashikiSearchFocus = Notification.Name("dev.kawaken.zashiki.searchFocus")
}

// NOTE: I am moving all of these to Notification.Name extensions over time. This
// namespace was the old namespace.
extension Zashiki.Notification {
    /// Used to pass a configuration along when creating a new tab/window/split.
    static let NewSurfaceConfigKey = "dev.kawaken.zashiki.newSurfaceConfig"

    /// Posted when a new split is requested. The sending object will be the surface that had focus. The
    /// userdata has one key "direction" with the direction to split to.
    static let zashikiNewSplit = Notification.Name("dev.kawaken.zashiki.newSplit")

    /// Close the calling surface.
    static let zashikiCloseSurface = Notification.Name("dev.kawaken.zashiki.closeSurface")

    /// Focus previous/next split. Has a SplitFocusDirection in the userinfo.
    static let zashikiFocusSplit = Notification.Name("dev.kawaken.zashiki.focusSplit")
    static let SplitDirectionKey = zashikiFocusSplit.rawValue

    /// Goto tab. Has tab index in the userinfo.
    static let zashikiGotoTab = Notification.Name("dev.kawaken.zashiki.gotoTab")
    static let GotoTabKey = zashikiGotoTab.rawValue

    /// New tab. Has base surface config requested in userinfo.
    static let zashikiNewTab = Notification.Name("dev.kawaken.zashiki.newTab")

    /// New window. Has base surface config requested in userinfo.
    static let zashikiNewWindow = Notification.Name("dev.kawaken.zashiki.newWindow")

    /// Present terminal. Bring the surface's window to focus without activating the app.
    static let zashikiPresentTerminal = Notification.Name("dev.kawaken.zashiki.presentTerminal")

    /// Toggle fullscreen of current window
    static let zashikiToggleFullscreen = Notification.Name("dev.kawaken.zashiki.toggleFullscreen")
    static let FullscreenModeKey = zashikiToggleFullscreen.rawValue

    /// Notification sent to toggle split maximize/unmaximize.
    static let didToggleSplitZoom = Notification.Name("dev.kawaken.zashiki.didToggleSplitZoom")
    ...
    static let didResizeSplit = Notification.Name("dev.kawaken.zashiki.didResizeSplit")
    ...
    static let didEqualizeSplits = Notification.Name("dev.kawaken.zashiki.didEqualizeSplits")

    /// Notification that renderer health changed
    static let didUpdateRendererHealth = Notification.Name("dev.kawaken.zashiki.didUpdateRendererHealth")

    /// Notifications related to key sequences
    static let didContinueKeySequence = Notification.Name("dev.kawaken.zashiki.didContinueKeySequence")
    static let didEndKeySequence = Notification.Name("dev.kawaken.zashiki.didEndKeySequence")
    static let KeySequenceKey = didContinueKeySequence.rawValue + ".key"

    /// Notifications related to key tables
    static let didChangeKeyTable = Notification.Name("dev.kawaken.zashiki.didChangeKeyTable")
