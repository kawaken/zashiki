...
extension Zashiki {
    ...
    struct Terminal: View {
        @EnvironmentObject private var ghostty: Zashiki.App
            ...
            ZStack {
                ...
                GeometryReader { geo in
                    ...
                    SurfaceRepresentable(view: surfaceView, size: geo.size)
                        ...
                        .focusedValue(\.zashikiSurfacePwd, surfaceView.pwd)
                        .focusedValue(\.zashikiSurfaceView, surfaceView)
                        .focusedValue(\.zashikiSurfaceCellSize, surfaceView.cellSize)
                    #if canImport(AppKit)
                ...
                .zashikiSurfaceView(surfaceView)
    ...
    struct SurfaceResizeOverlay: View {
        ...
        let overlay: Zashiki.Config.ResizeOverlay
        let position: Zashiki.Config.ResizeOverlayPosition
        let duration: UInt
    ...
    struct SurfaceSearchOverlay: View {
        ...
        var body: some View {
            GeometryReader { geo in
                HStack(spacing: 4) {
                    ...
                    .onExitCommand {
                        if searchState.needle.isEmpty {
                            ...
                            Zashiki.moveFocus(to: surfaceView)
                        }
                    }
#endif
                ...
                .onReceive(NotificationCenter.default.publisher(for: .zashikiSearchFocus)) { notification in
    ...
    /// A surface is terminology in Zashiki for a terminal surface, or a place where a terminal is actually drawn
    ...
    /// libghostty, usually from the Zashiki configuration.
...
private struct ZashikiSurfaceViewKey: EnvironmentKey {
    static let defaultValue: Zashiki.SurfaceView? = nil
}

private struct ZashikiLastFocusedSurfaceKey: EnvironmentKey {
    ...
    static let defaultValue: Weak<Zashiki.SurfaceView>? = nil
}

extension EnvironmentValues {
    var zashikiSurfaceView: Zashiki.SurfaceView? {
        get { self[ZashikiSurfaceViewKey.self] }
        set { self[ZashikiSurfaceViewKey.self] = newValue }
    }

    var zashikiLastFocusedSurface: Weak<Zashiki.SurfaceView>? {
        get { self[ZashikiLastFocusedSurfaceKey.self] }
        set { self[ZashikiLastFocusedSurfaceKey.self] = newValue }
    }
}

extension View {
    func zashikiSurfaceView(_ surfaceView: Zashiki.SurfaceView?) -> some View {
        environment(\.zashikiSurfaceView, surfaceView)
    }

    /// The most recently focused surface (can be currently focused if the surface is currently focused).
    func zashikiLastFocusedSurface(_ surfaceView: Weak<Zashiki.SurfaceView>?) -> some View {
        environment(\.zashikiLastFocusedSurface, surfaceView)
    }
}
...
extension FocusedValues {
    var zashikiSurfaceView: Zashiki.SurfaceView? {
        get { self[FocusedZashikiSurface.self] }
        set { self[FocusedZashikiSurface.self] = newValue }
    }

    struct FocusedZashikiSurface: FocusedValueKey {
        typealias Value = Zashiki.SurfaceView
    }

    var zashikiSurfacePwd: String? {
        get { self[FocusedZashikiSurfacePwd.self] }
        set { self[FocusedZashikiSurfacePwd.self] = newValue }
    }

    struct FocusedZashikiSurfacePwd: FocusedValueKey {
        typealias Value = String
    }

    var zashikiSurfaceCellSize: OSSize? {
        get { self[FocusedZashikiSurfaceCellSize.self] }
        set { self[FocusedZashikiSurfaceCellSize.self] = newValue }
    }

    struct FocusedZashikiSurfaceCellSize: FocusedValueKey {
