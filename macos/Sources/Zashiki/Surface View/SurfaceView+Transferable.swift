...
extension Zashiki.SurfaceView: Transferable {
    static var transferRepresentation: some TransferRepresentation {
        DataRepresentation(contentType: .zashikiSurfaceId) { surface in
    ...
    static func find(uuid: UUID) -> Self? {
        #if canImport(AppKit)
        guard let del = NSApp.delegate as? Zashiki.Delegate else { return nil }
        return del.zashikiSurface(id: uuid) as? Self
...
extension UTType {
    ...
    static let zashikiSurfaceId = UTType(exportedAs: "dev.kawaken.zashikiSurfaceId")
}

#if canImport(AppKit)
extension NSPasteboard.PasteboardType {
    /// Pasteboard type for dragging surface IDs.
    static let zashikiSurfaceId = NSPasteboard.PasteboardType(UTType.zashikiSurfaceId.identifier)
}
#endif
