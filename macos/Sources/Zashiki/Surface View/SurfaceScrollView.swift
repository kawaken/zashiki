...
/// Wraps a Zashiki surface view in an NSScrollView to provide native macOS scrollbar support.
///
/// ## Coordinate System
...
/// - `surfaceView`: The actual Zashiki renderer, positioned to fill the visible rect
class SurfaceScrollView: NSView {
    private let scrollView: NSScrollView
    private let documentView: NSView
    private let surfaceView: Zashiki.SurfaceView
    ...
    init(contentSize: CGSize, surfaceView: Zashiki.SurfaceView) {
        ...
        // so that our primary Zashiki renderer only needs to render the viewport.
        ...
        // Listen for scrollbar updates from Zashiki
        observers.append(NotificationCenter.default.addObserver(
            forName: .zashikiDidUpdateScrollbar,
            object: surfaceView,
            queue: .main
        ) { [weak self] notification in
    ...
    private func handleScrollbarUpdate(_ notification: Notification) {
        guard let scrollbar = notification.userInfo?[SwiftUI.Notification.Name.ScrollbarKey] as? Zashiki.Action.Scrollbar else {
            return
        }
