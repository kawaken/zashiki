...
extension Zashiki {
    ...
    class SurfaceView: OSSurfaceView, Codable, Identifiable {
        ...
        // detected with the set_password_input_cb on the Zashiki state.
        ...
        // Returns this surface's unique Zashiki Surface ID (same value
        // exposed to child processes as ZASHIKI_SURFACE_ID), or nil if the
        // surface has been closed.
        var zashikiSurfaceID: UInt64? {
        ...
        private(set) var surfaceModel: Zashiki.Surface?
        ...
        var scrollbar: Zashiki.Action.Scrollbar?
        ...
        init(_ app: ghostty_app_t, baseConfig: SurfaceConfiguration? = nil, uuid: UUID? = nil) {
            ...
            accessibilitySelectionCancellable = NotificationCenter.default
                ...
                .publisher(for: .zashikiSelectionDidChange)
            ...
            center.addObserver(
                ...
                name: Zashiki.Notification.didUpdateRendererHealth,
                object: self)
            center.addObserver(
                self,
                selector: #selector(zashikiDidContinueKeySequence),
                name: Zashiki.Notification.didContinueKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(zashikiDidEndKeySequence),
                name: Zashiki.Notification.didEndKeySequence,
                object: self)
            center.addObserver(
                self,
                selector: #selector(zashikiDidChangeKeyTable),
                name: Zashiki.Notification.didChangeKeyTable,
                object: self)
            center.addObserver(
                self,
            ...
            center.addObserver(
                self,
                selector: #selector(zashikiColorDidChange(_:)),
            ...
            guard let surface = surface else {
                self.error = Zashiki.Error.apiFailed
                return
            }
            self.surfaceModel = Zashiki.Surface(cSurface: surface)
        ...
        override func sizeDidChange(_ size: CGSize) {
            // Zashiki wants to know the actual framebuffer size... It is very important
        ...
        @objc private func zashikiDidContinueKeySequence(notification: SwiftUI.Notification) {
            guard let keyAny = notification.userInfo?[Zashiki.Notification.KeySequenceKey] else { return }
        ...
        @objc private func zashikiDidEndKeySequence(notification: SwiftUI.Notification) {
            DispatchQueue.main.async { [weak self] in
                self?.keySequence = []
            }
        }

        @objc private func zashikiDidChangeKeyTable(notification: SwiftUI.Notification) {
            guard let action = notification.userInfo?[Zashiki.Notification.KeyTableKey] as? Zashiki.Action.KeyTable else { return }
        ...
        @objc private func zashikiConfigDidChange(_ notification: SwiftUI.Notification) {
            // Get our managed configuration object out
            guard let config = notification.userInfo?[
                SwiftUI.Notification.Name.ZashikiConfigChangeKey
        ...
        @objc private func zashikiColorDidChange(_ notification: SwiftUI.Notification) {
            guard let change = notification.userInfo?[
                SwiftUI.Notification.Name.ZashikiColorChangeKey
            ] as? Zashiki.Action.ColorChange else { return }
        ...
        @objc private func zashikiBellDidRing(_ notification: SwiftUI.Notification) {
        ...
        override func mouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = Zashiki.zashikiMods(event.modifierFlags)
        ...
        override func otherMouseDown(with event: NSEvent) {
            guard let surface = self.surface else { return }
            let mods = Zashiki.zashikiMods(event.modifierFlags)
            let button = Zashiki.Input.MouseButton(fromNSEventButtonNumber: event.buttonNumber)
        ...
        override func mouseEntered(with event: NSEvent) {
            ...
            let mouseEvent = Zashiki.Input.MousePosEvent(
                x: pos.x,
        ...
        override func pressureChange(with event: NSEvent) {
            ...
            // Notify Zashiki first. We do this because this will let Zashiki handle
        ...
        override func keyDown(with event: NSEvent) {
            ...
            let translationModsZashiki = Zashiki.eventModifierFlags(
                mods: ghostty_surface_key_translation_mods(
                    surface,
                    Zashiki.zashikiMods(event.modifierFlags)
                )
            )

            // There are hidden bits set in our event that matter for certain dead keys
            // so we can't use translationModsZashiki directly. Instead, we just check
            // for exact states and set them.
            var translationMods = event.modifierFlags
            for flag in [NSEvent.ModifierFlags.shift, .control, .option, .command] {
                if translationModsZashiki.contains(flag) {
            ...
            if markedTextBefore,
               ...
               list.count > 0 {
                for text in list {
                    if Zashiki.SurfaceView.shouldSuppressComposingControlInput(
                        text,
                        composing: composing
                    ) {
                        continue
            ...
            if let list = keyTextAccumulator, list.count > 0 {
                ...
                _ = keyAction(
                    ...
                    event: event,
                    translationEvent: translationEvent,
                    text: translationEvent.zashikiCharacters,
                    composing: composing
                )
            }
        }
        ...
        override func performKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            let bindingFlags = surfaceModel.flatMap { surface in
                ...
                return (event.characters ?? "").withCString { ptr in
                    ...
                    return surface.keyIsBinding(zashikiEvent)
                }
            }
                ...
                if keySequence.isEmpty,
                   ...
                   bindingFlags.contains(.consumed) {
                    if let appDelegate = NSApp.delegate as? AppDelegate,
                       appDelegate.performZashikiBindingMenuKeyEquivalent(with: event) {
                        return true
                    }
                }
        ...
        private func keyAction(
            ...
            var key_ev = event.zashikiKeyEvent(action, translationMods: translationEvent?.modifierFlags)
            key_ev.composing = composing

            // For text, we only encode UTF8 if we don't have a single control
            // character. Control characters are encoded by Zashiki itself.
        ...
        private func shouldReplayCommittedPreeditKey(_ event: NSEvent) -> Bool {
            guard let key = Zashiki.Input.Key(keyCode: event.keyCode) else { return false }
            switch key {
        ...
        override func quickLook(with event: NSEvent) {
            ...
            // Zashiki coordinate system is top-left, convert to bottom-left for AppKit
        ...
        func showUserNotification(title: String, body: String, requireFocus: Bool = true) {
            ...
            content.categoryIdentifier = Zashiki.userNotificationCategory
            content.userInfo = [
        ...
        struct DerivedConfig {
            ...
            let scrollbar: Zashiki.Config.Scrollbar
            ...
            init(_ config: Zashiki.Config) {
                ...
                self.windowAppearance = .init(zashikiConfig: config)
        ...
        required convenience init(from decoder: Decoder) throws {
            // Decoding uses the global Zashiki app
...
extension Zashiki.SurfaceView: NSTextInputClient {
    ...
    func firstRect(forCharacterRange range: NSRange, actualRange: NSRangePointer?) -> NSRect {
        ...
        // Zashiki will tell us where it thinks an IME keyboard should render.
        var x: Double = 0
        var y: Double = 0
        ...
        // Zashiki coordinates are in top-left (0, 0) so we have to convert to
...
extension Zashiki.SurfaceView: NSServicesMenuRequestor {
...
extension Zashiki.SurfaceView: NSMenuItemValidation {
    func validateMenuItem(_ item: NSMenuItem) -> Bool {
        switch item.action {
        case #selector(pasteSelection):
            let pb = NSPasteboard.zashikiSelection
...
extension Zashiki.SurfaceView {
    ...
    /// this to copy styling information as well but we need to augment Zashiki core to
    /// expose that.
    ///
