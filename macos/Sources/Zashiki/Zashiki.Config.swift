...
extension Zashiki {
    ...
    class Config: ObservableObject {
        // The underlying C pointer to the Zashiki config structure. This
        ...
        /// in the Zashiki configuration. For example `keybind = cmd+q=quit` in Zashiki
        ...
        func keyboardShortcut(for action: String) -> KeyboardShortcut? {
            ...
            return Zashiki.keyboardShortcut(for: trigger)
        }
#endif

        // MARK: - Configuration Values

        /// For all of the configuration values below, see the associated Zashiki documentation for
        ...
        var macosCustomIcon: String {
            #if os(macOS)
            let defaultValue = NSString("~/.config/zashiki/Zashiki.icns").expandingTildeInPath
            guard let config = self.config else { return defaultValue }
            var v: UnsafePointer<Int8>?
...
#error("unsupported")
        ...
        var quickTerminalScreen: QuickTerminalScreen {
            ...
            let str = String(cString: ptr)
            return QuickTerminalScreen(fromZashikiConfig: str) ?? .main
        }
        ...
        var quickTerminalSpaceBehavior: QuickTerminalSpaceBehavior {
            ...
            let str = String(cString: ptr)
            return QuickTerminalSpaceBehavior(fromZashikiConfig: str) ?? .move
        }
        ...
        var commandPaletteEntries: [Zashiki.Command] {
            guard let config = self.config else { return [] }
            ...
            return buffer.map { Zashiki.Command(cValue: $0) }
        }
