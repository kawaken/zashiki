...
extension Zashiki {
    /// The manager that's responsible for updating shortcuts of Zashiki's app menu
    @MainActor
    class MenuShortcutManager {

        /// Zashiki menu items indexed by their normalized shortcut. This avoids traversing
        ...
        /// action string used for the Zashiki configuration.
        func syncMenuShortcut(_ config: Zashiki.Config, action: String?, menuItem: NSMenuItem?) {
        ...
        /// Zashiki keybind actions. This is important because it lets our surface dispatch
        /// bindings through the menu so they flash but also lets our surface override macOS built-ins
        /// like Cmd+H.
        func performZashikiBindingMenuKeyEquivalent(with event: NSEvent) -> Bool {
            ...
            // If we don't have an entry for this key combo, no Zashiki-owned
...
private extension Zashiki.MenuShortcutManager {
    /// Syncs a single menu shortcut for the given action. The action string is the same
    /// action string used for the Zashiki configuration.
    ///
    /// - Returns: Whether the menu item is updated and saved in ``menuItemsByShortcut``
    func updateMenuShortcut(_ config: Zashiki.Config, action: String?, menuItem menu: NSMenuItem) -> Bool {
        guard
            let action,
...
extension Zashiki.MenuShortcutManager {
    ...
    struct MenuShortcutKey: Hashable {
        ...
        init?(_ shortcut: KeyboardShortcut) {
            // Zashiki configured shortcuts are already normalized
            // in `Zashiki.keyboardShortcut(for:)`, see also gh-#12039
