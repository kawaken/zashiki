...
extension NSEvent {
    /// Create a Zashiki key event for a given keyboard action.
    ///
    ...
    func zashikiKeyEvent(
        ...
        key_ev.mods = Zashiki.zashikiMods(modifierFlags)
        key_ev.consumed_mods = Zashiki.zashikiMods(
    ...
    /// Returns the text to set for a key event for Zashiki.
    ///
    /// This namely contains logic to avoid control characters, since we handle control character
    /// mapping manually within Zashiki.
    var zashikiCharacters: String? {
        ...
        if characters.count == 1,
           let scalar = characters.unicodeScalars.first {
            ...
            // encoding directly within Zashiki's KeyEncoder.
            if scalar.value < 0x20 {
                return self.characters(byApplyingModifiers: modifierFlags.subtracting(.control))
            }

            // If we have a single value in the PUA, then it's a function key and
            // we don't want to send PUA ranges down to Zashiki.
