...
- **JSONスキーマは`gw list --json`の実行結果だけでなく、Goのソース（`cmd/gw/main.go`の`Result`/`Worktree`/`CleanupReport`型定義）を直接確認した方が正確だった。** 実行結果のサンプリングだけでは`detached`フィールド（今回観測した16件のworktreeには一件も無かった）や、`gw clean --json`（dry-runなしの実行）が返す`removed: []string`（`candidates`ではなくpathの配列）の形が分からなかった。幸い`go install`でインストール済みだったため`~/go/pkg/mod/github.com/kawaken/gw@<version>/cmd/gw/main.go`をそのまま読めた。
- **`Zashiki*.entitlements`にサンドボックス関連キーが無く（`com.apple.security.app-sandbox`なし）、`Process`の起動に制限はなかった。** 事前に確認しておいて正解だった。
- **`gw clean --json`（実行）を安全に試すため、`/tmp`に使い捨てのgitリポジトリを作って動作を見ようとしたが、Bashのパーミッションで拒否された。** 代わりにGoソースの型定義を読むことで構造を確定できたので、実害はなかった。
- **`GwClient`はテストのために`timeout`もDIできるようにした**（プラン時点では`binaryPathOverride`のみを想定していたが、タイムアウトのテストで20秒待つのは非現実的だったため）。テストは実在の`gw`を使わず、`/bin/sh`の使い捨てスクリプトを`binaryPathOverride`に渡すことで、正常系・非ゼロ終了・不正なJSON・タイムアウトの4パターンを実プロセス起動込みで検証できた。
- **キーボードショートカットは割り当てなかった。** ⌘⇧M（Toggle Markdown Preview）・⌘⇧F（Toggle Full Screen）と衝突しない空きキーが自明ではなく、メニュー項目からの起動で当面十分と判断した。
- **`TerminalView.swift`の`WorktreeStatusSplit`は`MarkdownPreviewSplit`のさらに外側に配置した**（左パネル・右パネルという別サイドなので、ネストしても混線しない）。
- 実機（Xcodeビルド）でのパネル表示・`gw clean`ボタンの実行確認はこのプラン更新時点では未実施。Issueの`needs-verification`ラベルで human 確認待ちにする。
