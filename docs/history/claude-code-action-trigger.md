...
### 完了: Phase 3のE2E確認、重大な副作用を発見・修正

テスト用Issue(#173、CHANGELOG.mdへの1行追記)でE2E確認したところ、
着手コメント・`wip`付与・PR自動作成(#174)・AutoMergeによる自動マージ・
PRマージ後の`wip`自動解除まで、Phase 3の一連の自動化がすべて正常に
動作した。BigQueryにもissue_number: 173, pr_number: 174が正しく
記録された。

その後、Issue #124自身の状態を確認したところ、`wip`ラベルが
`github-actions[bot]`によって誤って外されていたことが発覚した
(timeline APIで確認)。原因は`remove-wip-on-merge.yml`の実装:
PR本文の`#\d+`パターンを全部抽出して該当Issueの`wip`を外す実装に
していたため、Issue #124のような「複数のサブPRに渡って進行する
親Issue」で、各サブPRの本文に`## Issue\n#124`と書いていただけで、
まだ完了していない親Issueの`wip`がそのつど外れてしまっていた。
Issue #124の`wip`は手動で復元した。

再発防止として、`remove-wip-on-merge.yml`をPR本文の解析ではなく、
**ブランチ名が`claude/issue-<番号>-<タイムスタンプ>`のパターンに
一致するPRに限定**する実装に変更した。これにより、claude-code-action
経由で作られたPR以外(人間が手動で作業する通常のPR)には一切影響しない。

### 未着手

- 上記(スコープ限定修正)を反映した状態での再E2E確認
...
- `wip`ラベルは着手時に付与され、PRマージ時に外れる。ただし
  claude-code-action経由のPRに限定し、人間が手動で作業する通常のPR
  には影響しない (Phase 3、達成済み)
